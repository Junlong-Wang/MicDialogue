...
def sample_sequence(history, tokenizer, model, args, current_output=None):
    ...
    for i in range(args.max_length):
        ...
        input_ids = torch.tensor(instance["input_ids"], dtype=torch.long, device=args.device).unsqueeze(0)  # torch.Size([1, x])
        token_type_ids = torch.tensor(instance["token_type_ids"], dtype=torch.long, device=args.device).unsqueeze(0)

        logits, *_ = model(input_ids, token_type_ids=token_type_ids)  # logits: torch.Size([1, x, 13088])
        logits = logits[0, -1, :] / args.temperature  # logits: torch.Size([13088])
        logits = top_filtering(logits, top_k=args.top_k, top_p=args.top_p)  # 参数默认都是0，啥也没干
        probs = F.softmax(logits, dim=-1)

        prev = torch.topk(probs, 1)[1] if args.no_sample else torch.multinomial(probs, 1)  # 目前是随机采样啊
        if i < args.min_length and prev.item() in special_tokens_ids:  # 如果没到指定的长度且生成了特殊token
...
def main():
    ...
    parser.add_argument("--datapath", type=str, default="./data/dataset.json", help="Path of the dataset.")
    parser.add_argument("--out_path", type=str, default="result.json", help="Path of response generated.")
    parser.add_argument("--model_checkpoint", type=str, default="./runs/Jul07_16-58-28_DESKTOP-U7EJ8CH", help="Path, url or short name of the model")
    ...
    for instance in tqdm(dataset, mininterval=1):
        # print(instance)
        history = tokenize(instance['dialog'][:-1])
        ...
        prediction = {
            "history":instance['dialog'][:-1],
            "reference":instance['dialog'][-1],
            "prediction":out_text.replace(" ","")
        }

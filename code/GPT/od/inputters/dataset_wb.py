...
class WBDataset(Dataset):
    ...
    def __init__(self, data, knowledge, tokenizer, max_history=15, batch_first=True, lm_labels=True):
        self.data = data
    ...
    def __getitem__(self, index):
        ...
        return self.process(history, resposne, knowledge)

    def process(self, history, response, knowledge, with_eos=True):
        bos, eos, speaker1, speaker2,med_b,med_e= self.tokenizer.convert_tokens_to_ids(SPECIAL_TOKENS)
        ...
        if len(knowledge)>0:
            for entity in knowledge[:5]:
                # [] + [82, 5907, 8044, 1414] + [xxx]
                entities = entities + entity + [eos]
        entities = [med_b]+  entities  +[med_e]
        sequence = sequence[:-1] + [entities] + [sequence[-1]]

        # 在对话历史之前拼接上[MED]实体[SEP]实体[\MED]
        instance = {}
        instance["input_ids"] = list(chain(*sequence))
        # print(instance["input_ids"])
        # token_type_ids也要改，med部分改成MED的嵌入，这里是共享的嵌入，没有单独定义一个token_type嵌入矩阵

        # instance["token_type_ids"] = [bos] + [speaker2 if i % 2 else speaker1 for i, s in
        #                                       enumerate(sequence[1:]) for _ in s]
        # 每个子序列都只要跟他的第一个标识符一样就行了
        token_type_ids = [[s[0]] * len(s) for s in sequence]
        # print(token_type_ids)
        instance["token_type_ids"] = list(chain(*token_type_ids))
        instance["lm_labels"] = [-1] * len(instance["input_ids"])
        if self.lm_labels:
            # reponse正常，前面的history全置为-1
